...
class CutPaste:
    ...
    def paste_single_obj_with_prev(self, pcds, pcds_road, pcds_label, pcds_ins_label,
                                   prev_pcds, prev_pcds_road, prev_pcds_label_use, prev_pcds_ins_label, shifted_pcds):
        '''
        Input:
            pcds, (N, 4), 4 -> x, y, z, intensity
            pcds_road, (M, 4)
        ...
        if(len(pcds_obj) < 10):
            return pcds, pcds_label, pcds_ins_label, prev_pcds, prev_pcds_label_use, prev_pcds_ins_label, shifted_pcds
        ...
        for theta in theta_list:
            ...
            if (abs(u_fov[1] - u_fov[0]) < 8) and (abs(phi_fov[1] - phi_fov[0]) < 1) and (abs(theta_fov[1] - theta_fov[0]) < 1):
                # get valid fov
                fov_mask = self.occlusion_process(pcds, phi_fov, theta_fov)
                prev_fov_mask = self.occlusion_process(prev_pcds, phi_fov, theta_fov)
                in_fov_obj_mask = in_range(pcds_label[fov_mask], (1, 9))
                prev_in_fov_obj_mask = in_range(prev_pcds_label_use[prev_fov_mask], (1, 9))
                if in_fov_obj_mask.sum() < 3:
                    assert pcds.shape[0] == pcds_label.shape[0]
                    assert pcds.shape[0] == pcds_ins_label.shape[0]
    ...
    def __call__(self, pcds, pcds_label, pcds_ins_label,
        ...
        if paste_obj_num == 0:
            if shifted_pcds is not None:
                return pcds, pcds_label, pcds_ins_label,\
                    prev_pcds_list, prev_pcds_label_use_list, prev_pcds_ins_label_list,\
                    shifted_pcds, mapping_mat
            else:
                return pcds, pcds_label, pcds_ins_label
        else:
            if shifted_pcds is not None:
                ...
                prev_pcds_road = [prev_pcds_list[prev_pcds_label_use_list == i] for i in self.road_idx]
                prev_pcds_road = np.concatenate(prev_pcds_road, axis=0)
                
                pcds_new = pcds.copy()
                pcds_label_new = pcds_label.copy()
                pcds_ins_label_new = pcds_ins_label.copy()
                
                prev_pcds_new = prev_pcds_list.copy()
                prev_pcds_label_use_new = prev_pcds_label_use_list.copy()
                prev_pcds_ins_label_new = prev_pcds_ins_label_list.copy()
                shifted_pcds_new = shifted_pcds.copy()
                
                for i in range(paste_obj_num):
                    pcds_new, pcds_label_new, pcds_ins_label_new,\
                        prev_pcds_new, prev_pcds_label_use_new, prev_pcds_ins_label_new, shifted_pcds_new\
                            = self.paste_single_obj_with_prev(pcds_new, pcds_road, pcds_label_new, pcds_ins_label_new,\
                                prev_pcds_new, prev_pcds_road, prev_pcds_label_use_new, prev_pcds_ins_label_new, shifted_pcds_new)
                    # check
                    mapping_mat = None
                return pcds_new, pcds_label_new, pcds_ins_label_new, prev_pcds_new, prev_pcds_label_use_new, prev_pcds_ins_label_new, shifted_pcds_new, mapping_mat
            else:
                pcds_road = [pcds[pcds_label == i] for i in self.road_idx]
                pcds_road = np.concatenate(pcds_road, axis=0)
